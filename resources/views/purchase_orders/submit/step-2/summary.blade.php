    ...
    <table class="table table-standard table-summary">
        ...
        <template v-for="cost in additionalCosts">
            <tr class="row-added-costs">
                <td class="col-title">
                    @{{ cost.name }}
                    <button type="button" class="close" aria-label="Close" @click="removeAdditionalCost(cost)"><span
        ...
        <tr class="row-inputs">
            ...
            <td class="col-amount">
                <number-input :model.sync="newCostAmount" :placeholder="'amount'"
                              :class="['form-control']"></number-input>
            </td>
            <td class="col-currency">
                <select-picker :options="[{value: '%', label: '%'}, {value: currencySymbol, label: currencySymbol }]"
                               :name.sync="newCostType"></select-picker>
            </td>
        </tr>
        <tr v-show="canAddNewCost" class="row-add-button">
            ...
            <td></td>
            <td>
                <button type="button" class="btn btn-small btn-add-cost btn-outline-blue" @click="addAdditionalCost"><i
                        class="fa fa-plus"></i> Cost / Discount</button></td>
        </tr>
        <tr class="row-total">
